...
module.exports = class GitSync {
  ...
  async run() {
    try {
      ...
      if (
        (config.issue.node_id && config.issue.node_id.startsWith("PR_")) ||
  ...
  async createIssuesFromWorkItems(config) {
    ...
    // 1) run a WIQL query for items without “GitHub Issue”
    const conn = this.getConnection(config);
    const client = await conn.getWorkItemTrackingApi();
    const context = { project: config.ado.project };
    const wiql = {
      query:
        ...
        config.ado.wit +
        "' " +
        "AND NOT [System.Tags] CONTAINS 'GitHub Issue' " +
        "AND [System.ChangedDate] > @Today - 1",
    };
    const result = await client.queryByWiql(wiql, context);

    // 2) for each, create a GitHub issue
    for (const wiRef of result.workItems) {
      const wiObj = await client.getWorkItem(wiRef.id, [
        "System.Title",
        "System.Description",
        "System.Tags",
      ]);

      const octo = github.getOctokit(config.github.token);
      const [owner, repo] = config.GITHUB_REPOSITORY.split("/");
      const gh = await octo.rest.issues.create({
        owner,
        repo,
        title: wiObj.fields["System.Title"],
        body: new showdown.Converter().makeMarkdown(
          wiObj.fields["System.Description"] || ""
        ),
      });

      // 3) tag & link the ADO item back to the new issue
      const newTags =
        (wiObj.fields["System.Tags"] || "") +
        `GitHub Issue;GitHub Repo: ${config.GITHUB_REPOSITORY};` +
        `GitHub Issue #${gh.data.number};`;
      const patch = [
      ...
      await client.updateWorkItem([], patch, wiRef.id, config.ado.project);
    }
  }
